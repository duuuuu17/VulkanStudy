...
namespace lve {
	...
	{	
		...
		glm::vec3 w{ glm::normalize(direction) }; // forward
		glm::vec3 u{ glm::normalize(glm::cross(w, up)) }; // right
		glm::vec3 v{ glm::cross(w, u) }; // up
		// move the Camera position to the origin point
